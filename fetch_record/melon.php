...
define('CID',26);
define('COUNTRY','melon');
define('URL_VIL','http://melon-cirrus.sakura.ne.jp/sow/sow.cgi?m=a&v=');
define('URL_LOG','http://melon-cirrus.sakura.ne.jp/sow/sow.cgi?cmd=oldlog');
...
foreach($fetched_v as $item_vil)
{
  ...
  else
  {
    ...
    if(array_key_exists($wtmid,${'WTM_'.$rp}))
    {
      $village['wtmid'] = ${'WTM_'.$rp}[$wtmid];
    }
    else
    {
      echo 'NOTICE: '.$village['vno'].' has undefined winners message.->'.$wtmid;
      $village['wtmid'] = $data::TM_RP;
    }
  }
