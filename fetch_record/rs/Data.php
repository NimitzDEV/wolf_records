...
class Data
{
  ...
  const SKL_FRY_READ_G      =173; //妖兎 全秘密ログ閲覧
  const SKL_FRY_READ_G_DOG  =174; //月兎 妖兎+処刑突然死以外の死因を一度だけ防ぐ
  ...
  const SKL_SEA_NONE        =142; //スナッチャー 毎日一人を無能状態にする+終了時全員無能
  const SKL_SEA_EXPECT_DEAD =158; //グリムリーパー 指定した二人死亡
  const SKL_SEA_EXPECT_ALIVE=143; //ギャンブラー 指定した二人が生存
  const SKL_SEA_EXPECT_WIN  =167; //レギオン 村か狼陣営を指定、指定した陣営勝利
